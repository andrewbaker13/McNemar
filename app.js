...
function updateDiagnostics(data, stats) {
    ...
    const methodStatus = stats.primary.method === 'exact'
        ? 'good'
        : discordant < 25 ? 'caution' : 'good';
    const methodMessage = methodStatus === 'good'
        ? 'Selected method aligns with switcher volume.'
        : 'Consider switching to the exact binomial test for a cleaner Type I error rate.';
    ...
    container.innerHTML = `
        ...
        <div class="diagnostic-section">
            ...
            <ul>
                <li>Ensure each pair represents the same person/entity twice. Violations of matched sampling break McNemar’s guarantee.</li>
            </ul>
        </div>
    `;
}
